...
#     try:
#         retriever_instance=get_retriever()
#         doc=retriever_instance.invoke(query, k=3)
#         return "\n\n".join(d.page_content for d in docs) if docs else ""
#     except Exception as e:
#         logging.info(e)
...
# answer_llm= ChatGroq(model="llama-3.3-70b-versatile", 
#                        temperature=0.7)

# # lang graph state Data Structure
# class AgentState(TypedDict, total=False):
#     messages: list[BaseMessage]
#     route: Literal["rag", "web", "answer", "end"
#                    ]
...
#         # last message store in the bottom of list....for that just reverse it and get latest message
...
#     logging.info(f"router received web search info:{web_search_enabled}")
...
#         )
#     else:
#         system_prompt=( "**Web search is currently DISABLED.** You **MUST NOT** choose the 'web' route."
#             "If a query would normally require web search, you should attempt to answer it using RAG (if applicable) or directly from your general knowledge."
#             "\n\nChoose one of the following routes:"
#             "\n- 'rag': For queries about specific entities, historical facts, product details, procedures, or any information that would typically be found in a curated document collection, AND for queries that would normally go to web search but web search is disabled."
#             "\n- 'answer': For very simple, direct questions you can answer without any external lookup (e.g., 'What is your name?')."
...
#         result.route="rag"
#         router_override_reason="web search disabled by user; redirected to RAG."
#         logging.info("Router decision changed from 'web' to 'rag' and web search is disabled")

#     logging.info(f"Router final decision:{result.route}, reply (if 'end'):{result.reply}")
...
#         out["initial_router_decision"]=initial_router_decision
#         out["router_override_decision"]=router_override_reason

#     if result.route=="end":
#         out["messages"]=state["messages"]+[AIMessage(content=result.reply or "Hello")]  

#     logging.info("Existing router_node")
#     return out  

# # NODE 2.....rag lookup
# def rag_node(state:AgentState,config:RunnableConfig)->AgentState:
#     logging.info("Entering rag node")
#     #extract query
#     query=next((m.content for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), "")
#      # MODIFIED: Get web_search_enabled directly from the config
#     web_search_enabled = config.get("configurable", {}).get("web_search_enabled", True) # <-- CHANGED LINE
...
#     if chunks.startswith("RAG_ERROR::"):
#         logging.info(f"RAG Error :{chunks}, checking web search enabled status")

#         #if rag falls and web search enabled
#         next_route="web" if web_search_enabled else "answer"
#         return {**state, "rag": "", "route": next_route}
    
#     if chunks:
#         logging.info("Retrieved RAG chunks: {chunks[:500]}....")
#     else:
#         logging.info("RAG cannot retrieve chunks")
...
#     verdict: RagJudge=judge_llm.invoke(judge_messages)
#     logging.info(f"RAG judge verdict: {verdict.sufficient}")
#     logging.info("Existing rag_node")
...
#     else:
#         next_route="web" if web_search_enabled else "answer"
#         logging.info(f"RAG nor sufficient web search enabled:{web_search_enabled}.Next route:{next_route}")
...
# # Node 3  web search
# def web_node(state:AgentState,config:RunnableConfig)->AgentState:
#     logging.info("Entering Web node")
#     query=next((m.content for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), "")
#      # Check if web search is actually enabled before performing it
#     # MODIFIED: Get web_search_enabled directly from the config
#     web_search_enabled = config.get("configurable", {}).get("web_search_enabled", True) # <-- CHANGED LINE
#     logging.info(f"router received web search info:{web_search_enabled}")
#     if not web_search_enabled:
#         logging.info("Web search node entered but search is disabled")
#         return {
#             **state,
#              "web": "Web search was disabled by user","route":"answer"
#         } 
#     logging.info(f"web search query:{query}")
#     snippets=web_search_tool.invoke(query)

#     if snippets.startswith("WEB_ERROR::"):
#         logging.info(f"Web Error:{snippets}, Predicting to answer with limited info")
#         return {**state,"web":"","route":"answer"}
#     logging.info("Web snippets retrieved : {snippets[:200]}")
#     logging.info("Existing web_node")
#     return {**state,"web":snippets,"route":"answer"}

# # Node 4 Final answer
# def answer_node(state:AgentState)->AgentState:
#     logging.info("Entering Answered Node")
